...
## Overview
This feature adds a **Quick View** button to product cards on the PLP, triggering a popup with product details (images, variant picker, price, and **Add to Cart** button).  
It leverages the **Shopify Dawn** theme’s *Quick Add* feature with customizations for button placement, typography, and popup element ordering.
...
### Customization
- **Typography**: Configurable font size *(12–20 px)*, weight *(regular, medium, semibold, bold)*, and text case *(default, uppercase)* for the Quick View button.  
...
- Added schema for:
  ...
  - `quick_view_image_position`, `quick_view_price_position`,  
    `quick_view_variant_picker_position`, `quick_view_buy_buttons_position`:  
    Control popup element order *(1st–4th)*.
...
- **Responsive Design**: Media queries adjust padding and layout for mobile *(≤ 750 px)* and desktop.
...
## Testing
1. Verify button visibility and placement on product cards.
2. Test popup content (images, variants, price, Add to Cart) across products.
3. Check responsiveness on mobile, tablet, and desktop.
4. Ensure typography settings apply correctly to the button.
5. Validate element order changes in the popup via theme settings.
